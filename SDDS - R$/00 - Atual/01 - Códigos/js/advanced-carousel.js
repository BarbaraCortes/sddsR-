(function($){
	...
	lSAdvancedSlider.prototype.slideLeft = function(numberOfSlides)
	{
		...
		if((parseInt(parent.wrapper.css("left").replace("px",""))) == -indice*(parent.singleImageWidth))
		{
		...
		if((parseInt(parent.wrapper.css("left").replace("px",""))) < -4*(parent.singleImageWidth))
		{
			...
			var originalWaitTime = parent.waitTime;
			parent.waitTime = 0;
			parent.wrapper.animate({"left" : "+="+5*parent.singleImageWidth+"px"}, parent.waitTime);
			parent.waitTime = originalWaitTime;
	...
	
	{
			...
			var originalWaitTime = parent.waitTime;
			parent.waitTime = 0;
			parent.wrapper.animate({"left" : "-="+(indice+1)*parent.singleImageWidth+"px"}, parent.waitTime);
			parent.waitTime = originalWaitTime;
		}}
